...
@if (session('message-xa'))
  <div class="alert alert-success">
      ...
      <input id='message-xa' type = 'hidden' value="{{ session('message-xa') }}" />
  </div>
@endif
@if (session('error-xa'))
<div class="alert alert-danger">
    ...
    <input id='error-xa' type = 'hidden' value="{{ session('error-xa') }}" />
</div>
@endif
          ...
          <?php if ($page_xa >= 2) $total = ($page_xa - 1) * 6 + 1 ?>
            @foreach($xaphuong as $value)
              <tr class="data-row">
                  ...
                  <td id="typeq"><p class = 'text'>{{ $value->maxptr }}</p></td>
                  <td><p class = 'text'>{{ $value->maqh }}</td>
